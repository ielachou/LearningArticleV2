...
class PHCAgent:
	def __init__(self, initialStrategy=(0.5, 0.5), nb_actions=2, nb_states=1, gammma=0.9, delta=0.0001):
		...
		self.pi = [list(initialStrategy) for _ in range(nb_states)]
	...
	def initialSelfStrategy(self, s):
		for i in range(self.lengthOfAction):
			self.pi[s][i] = 1 / self.lengthOfAction
	...
	def act(self, s):
		if np.random.random() < self.EPSILON:
			...
			self.currentAction = np.random.choice(self.actions, p=self.pi[s])
		return self.currentAction

	def chooseActionWithFixedStrategy(self, s):
		self.currentAction = self.actions[generateRandomFromDistribution(self.pi[s])]
	...
	def updateActionValues(self, obs, nextObs):
		self.q_values[obs][self.currentAction] = (1 - self.alpha) * self.q_values[obs][self.currentAction] \
		                                         + self.alpha * (self.currentReward + self.gamma * np.max(
			self.q_values[nextObs][:]))

	def updateStrategy(self, s):
		self.maxAction = np.argmax(self.q_values[s])
		for i in range(self.lengthOfAction):
			self.deltaAction[i] = np.min([self.pi[s][i], self.delta / (self.lengthOfAction - 1)])
		...
		for i in range(self.lengthOfAction):
			self.pi[s][i] += self.deltaActionTop[i]
