...
class WoLFAgent():
	...
	def __init__(self, gamma=0.999, alpha=0.1, delta=0.0001, actions=None, nb_states=1, high_delta=0.004,
	             low_delta=0.002):
		random.seed(25)
		self.alpha = alpha
	...
	def observe(self, reward, obs, nextobs):
		...
		self.q_values[obs][self.last_action_id] = ((1.0 - self.alpha) * self.q_values[obs][self.last_action_id]) \
		                                          + (self.alpha * (
					reward + self.gamma * (self.q_values[nextobs].max())))
	...
	def _update_pi_average(self, s):
		...
		for aidx, _ in enumerate(self.pi[s]):
			self.pi_average[s][aidx] = self.pi_average[s][aidx] + (1 / self.conter) * (
						self.pi[s][aidx] - self.pi_average[s][aidx])
